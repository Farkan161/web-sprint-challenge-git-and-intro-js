...
function getArtistByIndex(_artists, id) {
  // array[index].artist = artist,

  if (id >= _artists.length) return "index out of bounds";

  return `the artist at index ${id} is ${_artists[id].name}`;
}

// console.log(getArtistByIndex(artists, 0));
// console.log(getArtistByIndex(artists, 100));
...
function listOfNames(_artists) {
  /*Your Code Here*/
  let newArtistsArry = [..._artists]; // copy array

  for(let x = 0; x < newArtistsArry.length; ++x) {
    const tempName = newArtistsArry[x].name;
    newArtistsArry[x] = tempName;
  }

  return newArtistsArry;
}
...
const remove = (arr, index) => {
  if (index > -1) {
    arr.splice(index, 1) // from 1 element from array stating from "index"
  }
}

function removeArtist(_artists, index) {
  let newArtistsArry = [..._artists]; // copy of an array

  remove(newArtistsArry, index);

  return newArtistsArry;
}

// 1 second I hear
const beforeRemovingIndex0 = listOfNames(artists);
const afterRemovingIndex0 = removeArtist(beforeRemovingIndex0, 5);

console.log("before: ", beforeRemovingIndex0, " size of Array: ", beforeRemovingIndex0.length);
console.log("after: ",afterRemovingIndex0, " size of Array: ", afterRemovingIndex0.length);
...
function addArtist(artists, name, years, genre, nationality, bio) {
  ...
  newArtistsArry.push(artist);
  
  return newArtistsArry;
}


console.log(addArtist(artists, "Fareed", 67, "Political", "USA", "He is a politician"));
...
function lotsOfArt(artists) {
  ...
  for(let i = 0; i < artists.length; i++) {
    if(artists[i].paintings >= 100) {
      lotsMoreArt.push(artists[i].name);
    }
  }
...
console.log("============== ", artistInfo(artists, "Fake Name"));
...
function artistByCountry(artists,filterTerm) {
  const filteredArr = [];

  for(let i = 0; i < artists.length; i++) {
    if(artists[i].nationality === filterTerm) {
      filteredArr.push(artists[i].name);
    }
  }
